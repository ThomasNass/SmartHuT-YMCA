...
const History = (props) => {
    ...
    let showHistory = !props.showClimate ? null : `${styles.hide}`;
        ...
        if (!props.showClimate) {
            const GetHistoryForRoom = async () => {
                ...
                for (let i = 0; i < devicesInRoom.length; i++) {
                    let historyInfo = await GetTheLog(
                        devicesInRoom[i].id,
                        token
                    );
